...
export interface EconomyUser {
  ...
  /** Купленный автомобиль — снимает аренду вела с UI и укорачивает КД курьера. */
  ownedCarId?: string;
  ...
  /** Баланс симки (пополнение в магазине); тариф курьера списывается отсюда. */
  simBalanceRub?: number;
  /** До какого момента оплачен месячный тариф сим для курьера (+30 суток после оплаты). */
  ...
  /** Последняя обработанная дата экономики по МСК (YYYY-MM-DD) — пассив/стрик тир-3. */
