...
export function platePrestigeRulesTableLines(): string[] {
  ...
  const regions = Object.entries(REGION_SCORES)
    ...
    .join(", ");
  const extra = Object.entries(DIGIT_EXTRA_BONUSES)
    .sort((a, b) => b[1] - a[1])
    .map(([digits, score]) => `${digits}:+${score.toLocaleString("ru-RU")}`)
    .join(", ");
  return [
    "`Источник` | `Очки`",
    ...
    "`Цифры 001/007` | `+2 500`",
    "`Цифры тройка XXX` | `+3 000`",
    "`Цифры круглая сотня (100…900)` | `+1 500`",
    "`Цифры палиндром (X Y X)` | `+1 200`",
    "`Цифры простой повтор (две одинаковые)` | `+500`",
    "`Цифры доп. бонус` | `по таблице ниже (поверх базы)`",
    "`Регион` | `по таблице`",
    ...
    "",
    "**Цифры — база:**",
    "001 и 007: **+2 500**. Тройка (111…999): **+3 000**. Круглая сотня (100, 200…900): **+1 500**. Палиндром (121, 343…): **+1 200**. Две одинаковые цифры: **+500**.",
    "",
    "**Цифры — доп. бонус (складывается с базой тройки):**",
    extra,
    "",
    "**Статусные серии (все):**",
    statusSeries,
