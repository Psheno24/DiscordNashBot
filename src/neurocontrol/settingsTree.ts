  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        "**ИП** — комиссия при выводе с баланса бизнеса на личный счёт и еженедельный налог с капитала бизнеса (по понедельникам, МСК).",
      ].join("\n\n"),
    );
}
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        "Удерживается с суммы, которая **зачисляется на личный счёт**: смены (кроме нелегала), ежедневные оклады офиса и ИП, кнопки «Связь» / «Совещание» у офиса.",
        "",
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        "Списывается с баланса бизнеса ИП по **понедельникам** (полночь МСК), в казну. Один раз за календарный понедельник.",
      ].join("\n\n"),
    );
}
  ...
  if (id === NEURO_TAX_OPEN_IP_CAP) {
    ...
    modal.addComponents(
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        new TextInputBuilder()
          .setCustomId("pct")
          .setLabel("Процент в неделю (пн, МСК), 0–100")
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setMaxLength(8)
